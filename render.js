...
!function(){
	"use strict";
	window._mintMap = {setOpacity:()=>{}, metadata: [], listOfLayersNotAdded:[], variableArray:[], mapInspect:{}, uiLoaded:false, autocomplete:{}, toggleClass:()=>{}};
    window.loadMapLayers = function(mapboxgl){
			...
			set(target, key, value) {
				...
				if (key === 'metadata') {
	                ...
	                for (var i = 0; i < json.layerNames.length; i++) {
	                    if (!json.hasData[i]) {
	                        window._mintMap.listOfLayersNotAdded.push({label:json.layerNames[i] + " (No data)", value: json.layerNames[i], id: json.layerIds[i], hasData: json.hasData[i], source: json.sourceLayers[i], file:""});    
	                    }else{
	                        // if (json.layerNames[i] != window.__defaultLayerName) {
	                        window._mintMap.listOfLayersNotAdded.push({label:json.layerNames[i], value: json.layerNames[i], id: json.layerIds[i], hasData: json.hasData[i], source: json.sourceLayers[i], file: json.layers[i].file});
	                        // }
	                    }
	                }
        ...
        window._mintMap.onVariablesChanged = function (variables) {
        	...
        	Object.assign(window._mintMap.variableArray,[]);
        	variables.forEach(function (ele, idx) {
		...
		window._mintMap.setOpacity = function(layerId, value) {
        ...
        function removeInspectLayers(curLayerName) {
            if ('layers' in window._mintMap.mapInspect.options.queryParameters) {
                ...
                if (idx > -1) {
                    window._mintMap.mapInspect.options.queryParameters['layers'].splice(curLayerName,1);
                }
            }
        }
        ...
        function loadLayerFromJson(obj) {
            ...
            // let rasterLayerId = curLayerName + "_raster";
        ...
        function updateShowAllDiv(isClick = true) {
            ...
            for (var i = 0; i < window._mintMap.listOfLayersNotAdded.length; i++) {
                var newLayer = document.createElement('li');
                newLayer.innerHTML = "<a class='tag " + (window._mintMap.listOfLayersNotAdded[i].hasData ? "with-data-tag":"no-data-tag") + "' data-layer-id='"+ window._mintMap.listOfLayersNotAdded[i].id +"' data-has-data='" + (window._mintMap.listOfLayersNotAdded[i].hasData ? "true":"false") + "' data-source-layer='" + window._mintMap.listOfLayersNotAdded[i].source + "' data-file='"+ window._mintMap.listOfLayersNotAdded[i].file +"'>" + window._mintMap.listOfLayersNotAdded[i].value + "<div class='tag_close tag_add'></div></a>";
		...
		function createProperitesPanel(layers, layersPropertyList, layersIds, layerNames, haveData, haveTimeline ) {
            ...
            for (var i = 0; i < layersIds.length; i++) {
                ...
                layerProperty.innerHTML = "<h3>" + name + " Layer Properties</h3>" 
                                         ...
                                         + "<h4>Opacity</h4>" 
                                         + "<div class='control'>" 
                                         + "<input type='range' min='1' max='100' value='80' class='slider opacity-slider' oninput='window._mintMap.setOpacity(\""+id+"\", this.value)'>"
                                         + "</div>";
                if (haveTimeline[i]) {
